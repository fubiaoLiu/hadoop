...
public class NameNodeRpcServer implements NamenodeProtocols {
  ...
  public NameNodeRpcServer(Configuration conf, NameNode nn)
    ...
    // 这里面有很多接口，包括管理block、创建目录、创建文件、设置权限等等的一大堆接口
    // ClientNameNode***表示是用于client和namenode之间通信的接口
    ...
    // 这里面也是一大堆接口，这个是datanode和namenode通信的接口
    // 主要是注册datanode、发送心跳、block report等一些接口
    ...
    // 上面这一大堆都是提供的接口，每个service负责一些接口
    // 从配置中获取rpc server的地址，主要是rpc server监听的端口号，默认未设置，使用rpc-address的8020端口
    ...
    if (serviceRpcAddr != null) {
      ...
      // 创建DataNode和NameNode通信的RPC.Server
      ...
      // 将上面的rpc service交给serviceRpcServer管理
    ...
    // 创建客户端和NameNode通信的RPC.Server，默认端口是8020
