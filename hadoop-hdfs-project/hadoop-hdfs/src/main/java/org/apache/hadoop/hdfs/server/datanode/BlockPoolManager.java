  ...
  private void doRefreshNamenodes(
    ...
    synchronized (this) {
      ...
      if (!toAdd.isEmpty()) {
        ...
        // 这里创建的BPOfferService就是表示一个nameservice，一个nameservice有两个namenode（active、standby）
        // 这里初始化BlockPoolManager的时候，会根据配置文件中的nameservice配置创建对应的BPOfferService
        // 普通的HA架构只有一个BPOfferService，联邦架构才会有多个。
        // 一个BPOfferService中有两个BPServiceActor，对应两个namenode。BPServiceActor实现了Runnable接口。
      ...
      // 启动那些BPServiceActor线程，线程中会向namenode进行注册，后面会定时发送心跳和汇报block report
      startAll();
    }
