...
class BPServiceActor implements Runnable {
  ...
  private void connectToNNAndHandshake() throws IOException {
    ...
    // 校验拿到的namespace信息，如果是第一次连接还会初始化BlockPool
  ...
  private void offerService() throws Exception {
    ...
    while (shouldRun()) {
      try {
        ...
        // 判断当前是不是应该发送心跳了，看下次发送心跳时间是否已经到了
        ...
        if (sendHeartbeat) {
          ...
          if (!dn.areHeartbeatsDisabledForTests()) {
            ...
            // 根据心跳返回的数据，更新当前BPOfferService的状态（比如感知到NameNode发生了状态切换：active -> standby）
        ...
        if ((fullBlockReportLeaseId != 0) || forceFullBr) {
          // 汇报block report，默认每6个小时汇报一次
          // 当前时间 + 随机值（小于6小时）（如果resetBlockReportTime为true）
          // 或
          // （当前时间 - 上次执行时间 + 6小时） / 6小时 * 6小时
  ...
  public void run() {
    ...
    try {
      while (true) {
        ...
        try {
          // setup storage
          // 连接到NameNode进行握手，这里会从NameNode获取namespace信息，并且进行注册
      ...
      while (shouldRun()) {
        try {
          // 这里会每隔一段时间就发送心跳、汇报block report
          // 这个方法里还有一个while true死循环
          offerService();
        } catch (Exception ex) {
