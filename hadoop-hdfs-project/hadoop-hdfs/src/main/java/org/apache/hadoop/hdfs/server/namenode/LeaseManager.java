...
public class LeaseManager {
  ...
  private final NavigableSet<Lease> sortedLeases = new TreeSet<>(
      new Comparator<Lease>() {
        ...
        public int compare(Lease o1, Lease o2) {
          // 按照最后更新时间进行排序，如果最后更新时间一样，按照客户端进行排序
  ...
  synchronized Lease addLease(String holder, long inodeId) {
    ...
    if (lease == null) {
      ...
      // leases是个TreeMap，按照客户端进行排序
      leases.put(holder, lease);
      // sortedLeases是个TreeSet，按照lease进行排序
      sortedLeases.add(lease);
    } else {
      renewLease(lease);
    }
    // leasesById也是个TreeMap，按照inode的id进行排序
  ...
  synchronized void renewLease(Lease lease) {
    if (lease != null) {
      // renew就是修改最后更新时间，sortedLeases先remove再add是为了维持顺序
